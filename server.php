...
while (true) {
	...
	foreach ($changed as $changed_socket) {
		...
		while(socket_recv($changed_socket, $buf, 1024, 0) >= 1)
		{
			...
			$message_time = date("d/m/Y H:i:s") . substr((string)microtime(), 1, 3);
			//prepare data to be sent to client
			$pieces = explode(" ", $user_message);
			$size = count($pieces);
			for($i=0;$i<$size;$i++){
			...
			$response_text = mask(json_encode(array('type'=>'usermsg', 'message'=>$user_message, 'name'=>$user_name, 'time'=>$message_time, 'image'=>$image, 'link'=>$link, 'productname'=>$name, 'code'=>$code)));
